...
<x-app-layout>
    <div class="bg-gray-50 dark:bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 lg:py-12">
            ...
            <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <button onclick="openModalAddStore()" class="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 text-sm font-semibold rounded-lg transition active:scale-[0.99] whitespace-nowrap">
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                ...
                <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    <div class="relative w-full sm:max-w-xs">
                        ...
                        <input type="text" id="search-input" placeholder="Buscar producto…" class="w-full pl-9 pr-4 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                    </div>
                    <div class="flex items-center gap-3 ml-auto">
                        <a href="{{ route('stock.index', ['low_stock' => request('low_stock') ? null : 1]) }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border {{ request('low_stock') ? 'bg-red-50 dark:bg-red-900/20 border-red-300 dark:border-red-700 text-red-700 dark:text-red-400' : 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700' }} transition">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody id="table-body" class="divide-y divide-gray-100 dark:divide-gray-800">
                            ...
                            <tr class="stock-row hover:bg-gray-50 dark:hover:bg-gray-700/50 transition" data-name="{{ strtolower($item->name ?? '') }}" data-code="{{ strtolower($item->code ?? '') }}">
    ...
    <div id="add-modal" class="fixed inset-0 z-50 hidden" aria-modal="true" role="dialog">
        <div id="add-backdrop" onclick="closeAddModal()" class="absolute inset-0 bg-black/50 backdrop-blur-sm opacity-0 transition-opacity duration-200"></div>

        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div id="add-panel" class="relative w-full max-w-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl translate-y-4 opacity-0 transition-all duration-200 max-h-[90vh] flex flex-col">
                ...
                <div class="overflow-y-auto flex-1">
                    <form method="POST" action="{{ route('stock.store') }}" class="p-5 space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">
                                Artículo *
                            </label>
                            <select name="item_id" required class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg outline-none">
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" required min="0" placeholder="0" class="w-full px-3 py-2 text-lg font-bold text-center bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg outline-none">
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Motivo</label>
                            <input type="text" name="reason" placeholder="Carga inicial, compra…" class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg outline-none">
                        </div>
                        <div class="flex justify-end gap-3">
                            <button type="button" onclick="closeAddModal()" class="px-4 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg">Cancelar</button>
                            <button type="submit" class="px-5 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-semibold rounded-lg">Guardar</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    {{-- ===================== MODAL AJUSTAR ===================== --}}
    <div id="adjust-modal" class="fixed inset-0 z-50 hidden" aria-modal="true" role="dialog">
        <div id="adjust-backdrop" onclick="closeAdjustModal()" class="absolute inset-0 bg-black/50 backdrop-blur-sm opacity-0 transition-opacity duration-200"></div>

        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div id="adjust-panel" class="relative w-full max-w-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl translate-y-4 opacity-0 transition-all duration-200">
                ...
                <div class="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <button onclick="closeAdjustModal()" class="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <line x1="18" y1="6" x2="6" y2="18" />
                            <line x1="6" y1="6" x2="18" y2="18" /></svg>
                    </button>
                ...
                <div class="overflow-y-auto flex-1">
                    <form id="stock-form" method="POST" class="p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" id="modal-quantity" required min="0" placeholder="0" class="w-full px-3 py-2 text-lg font-bold text-center bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                        ...
                        <div>
                            ...
                            <input type="text" name="reason" id="modal-reason" placeholder="Ej. Inventario físico, merma, devolución…" class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                        </div>

                        <div class="border-t border-gray-100 dark:border-gray-700 pt-4">
                            <button type="button" id="toggle-config" class="text-xs text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1.5">
                                <svg class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" id="config-icon">
                            ...
                            <div id="config-section" class="hidden mt-3 space-y-3">
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        ...
                                        <input type="number" name="min_stock" id="modal-min-stock" min="0" placeholder="0" class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                                    </div>
                                    <div>
                                        ...
                                        <input type="text" name="location" id="modal-location" placeholder="Bodega A" class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <input type="text" name="notes" id="modal-notes" placeholder="Observaciones" class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                                </div>
                            </div>
                        </div>

                        <div class="flex items-center justify-end gap-3 pt-1">
                            <button type="button" onclick="closeAdjustModal()" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                Cancelar
                            </button>
                            <button type="submit" class="px-5 py-2 bg-gray-900 dark:bg-white hover:bg-gray-800 dark:hover:bg-gray-100 text-white dark:text-gray-900 text-sm font-semibold rounded-lg transition active:scale-[0.99]">
                                Guardar
                            </button>
                        </div>
                    </form>
    ...
    <script>
        ...
        function openStockModal(itemId, itemName, currentStockValue, minStock, location, notes) {
            currentStock = currentStockValue;

            document.getElementById('stock-item-name').textContent = itemName;
            document.getElementById('stock-current').textContent = currentStockValue;
            ...
            document.body.style.overflow = 'hidden';
            requestAnimationFrame(() => {
                adjustBackdrop.style.opacity = '1';
                adjustPanel.style.opacity = '1';
                adjustPanel.style.transform = 'translateY(0)';
            });
        }

        function closeAdjustModal() {
            adjustBackdrop.style.opacity = '0';
            adjustPanel.style.opacity = '0';
            adjustPanel.style.transform = 'translateY(1rem)';
            setTimeout(() => {
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape') {
                ...
                if (!adjustModal.classList.contains('hidden')) closeAdjustModal();
            }
        });
        ...
        const typeStyles = {
            entrada: {
                on: 'bg-green-50 dark:bg-green-900/30 border-green-400 dark:border-green-600 text-green-700 dark:text-green-300'
                , off: 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50'
            }
            , salida: {
                on: 'bg-red-50 dark:bg-red-900/30 border-red-400 dark:border-red-600 text-red-700 dark:text-red-300'
                , off: 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 hover:bg-gray-50'
            }
        };
        ...
        function updatePreview() {
            ...
            let result = type === 'entrada' ? currentStock + qty : currentStock - qty;
