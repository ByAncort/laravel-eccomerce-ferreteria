...
<x-app-layout>
                            ...
                            <tr class="item-row hover:bg-gray-50 dark:hover:bg-gray-700/50 transition" data-name="{{ strtolower($item->name) }}" data-code="{{ strtolower($item->code) }}" data-category="{{ $item->category }}" data-brand="{{ strtolower($item->brand ?? '') }}">
                                ...
                                <td class="px-4 sm:px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-100 to-blue-200 dark:from-blue-900 dark:to-blue-800 flex items-center justify-center text-blue-700 dark:text-blue-300 font-bold text-xs flex-shrink-0 overflow-hidden">
                                            @if($item->main_image)
                                ...
                                <td class="px-4 sm:px-6 py-4">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <a href="{{ route('items.edit', $item->id) }}" class="w-8 h-8 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 transition" title="Editar">
    ...
    {{-- ===================== MODAL ===================== --}}
    <div id="product-modal" class="fixed inset-0 z-50 hidden" aria-modal="true" role="dialog">
        {{-- Backdrop --}}
        <div
            id="modal-backdrop"
            onclick="closeModal()"
        ...
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div
                id="modal-panel"
                class="relative w-full max-w-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl translate-y-4 opacity-0 transition-all duration-200 max-h-[90vh] flex flex-col">

                {{-- Modal Header --}}
                <div class="flex items-center justify-between p-5 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                    <div>
                        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Nuevo Producto</h2>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Completa los campos para registrar un producto</p>
                    </div>
                ...
                <div class="overflow-y-auto flex-1">
                    <form action="{{ route('items.store') }}" method="POST" id="product-form" class="p-5 sm:p-6 space-y-4">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="Ej. Taladro Percutor 600W"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" name="code" id="code" value="{{ old('code') }}" placeholder="HER20240001"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label for="brand" class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Marca</label>
                                <input type="text" name="brand" id="brand" value="{{ old('brand') }}" placeholder="Ej. Bosch, Black+Decker"
                                    class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                            </div>
                            <div>
                                <label for="model" class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Modelo</label>
                                <input type="text" name="model" id="model" value="{{ old('model') }}" placeholder="Ej. GSB 600 RE"
                                    class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                            </div>
                        </div>

                        <div>
                            <label for="description" class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Descripción</label>
                            <textarea name="description" id="description" rows="3" placeholder="Características y detalles del producto..."
                                class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition resize-none">{{ old('description') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="number" name="selling_price" id="selling_price" required step="0.01" min="0" value="{{ old('selling_price') }}" placeholder="0.00"
                            ...
                            <div>
                                <label for="cost_price" class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Precio costo</label>
                                <input type="number" name="cost_price" id="cost_price" step="0.01" min="0" value="{{ old('cost_price') }}" placeholder="0.00"
                                    class="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 dark:focus:ring-blue-800 outline-none transition">
                            </div>
                        </div>

                        <div>
                            <label for="vendor_id" class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">Proveedor</label>
